...
public class BlazingArcInstance extends AbilityInstance implements Collidable {
	...
	private boolean rayCast(Location loc, double dist) {
		boolean hit = false;
		RayTraceResult ray = loc.getWorld().rayTrace(loc, loc.getDirection(), dist, FluidCollisionMode.ALWAYS, true, 0.4, (e) -> e instanceof LivingEntity && !e.getUniqueId().equals(user.getUniqueID()) && !Velocity.isAffected(e));
		if (ray != null) {
			if (ray.getHitEntity() != null) {
				Effects.damage((LivingEntity) ray.getHitEntity(), damage, this, false);
				Velocity.knockback(ray.getHitEntity(), loc.getDirection().setY(0).multiply(knockback), this);
				hit = true;
			}
			
			hit |= ray.getHitBlock() != null;
		}
